...
You can list all of the models that have been uploaded to a domain, and optionally filter by state:

.. code-block:: python

    # List all models in a domain
    models = model_store.list_models("my-domain")

    # List models with a given state
    prod_models = model_store.list_models("my-domain", state_name="production")

.. note::

    :code:`list_versions()` is deprecated; use :code:`list_models()` instead.

Models
------

The main thing you can do with a model is download or load it back. You can also retrieve information about a specific model, check whether it exists, query its states, and delete it from the registry.

.. code-block:: python

    # Check whether a model exists
    exists = model_store.model_exists("my-domain", "my-model")

    # Get information about a specific model
    meta_data = model_store.get_model_info("my-domain", "my-model")

    # Get the states that have been set for a model
    states = model_store.get_model_states("my-domain", "my-model")
